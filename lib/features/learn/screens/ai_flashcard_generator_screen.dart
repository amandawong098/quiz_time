...
import '../../../core/services/ai_flashcard_service.dart';
...
class _AIFlashcardGeneratorScreenState
  ...
  Future<void> _loadDynamicTopics({bool forceRefresh = false}) async {
    ...
    try {
      final topics = await AIFlashcardService.fetchDynamicFlashcardTopics(
      ...
      if (mounted) {
        setState(() {
          _dynamicTopics =
              AIFlashcardService.defaultFlashcardTopics.take(5).toList();
          _isLoadingTopics = false;
        });
      }
    }
      ...
      child: Column(
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText:
                  'Type flashcard topic, terms, or attach notes (PDF, TXT, Images)...',
              hintStyle: TextStyle(
                color:
  ...
  Widget _buildDynamicWrappedSuggestions() {
    final topics = (_dynamicTopics.isNotEmpty
            ? _dynamicTopics
            : AIFlashcardService.defaultFlashcardTopics)
        .take(5)
        .toList();
