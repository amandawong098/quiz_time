...
''';
  ...
  /// Generates a single new chapter intended to extend an existing course.
  static Future<Map<String, dynamic>> generateChapterForCourse({
    ...
    String? fileMimeType,
    ProcessedAttachment? attachment,
    String audience = 'All Audiences',
    String? modelName,
  }) async {
    final apiKey = await AIQuizService.getApiKey();
    if (apiKey == null || apiKey.isEmpty) {
      throw Exception(
        'Gemini API Key is missing. Please configure it in Gemini settings.',
      );
    }
    ...
    final effectiveFileName = fileName ??
        (filesNames != null && filesNames.isNotEmpty
            ? filesNames.join(', ')
            : null);

    List<String> candidateModels = [];
    try {
      final liveModels = await AIQuizService.listAvailableModelsForApiKey(apiKey);
      if (liveModels.isNotEmpty) {
        if (modelName != null && liveModels.contains(modelName)) {
          candidateModels.add(modelName);
        }
        final flashModels = liveModels.where((m) => m.contains('flash')).toList();
        final otherModels = liveModels.where((m) => !m.contains('flash')).toList();
        for (var m in [...flashModels, ...otherModels]) {
          if (!candidateModels.contains(m)) {
            candidateModels.add(m);
          }
    ...
    for (final currentModel in candidateModels) {
      try {
        debugPrint('Attempting AI chapter generation with model: $currentModel');
        final chapterData = await _executeChapterGeneration(
          modelName: currentModel,
          apiKey: apiKey,
        ...
        // Bind authentic figures if extracted
        if (attachment != null && attachment.extractedImageUrls.isNotEmpty) {
          final authenticUrls = attachment.extractedImageUrls;
          int fallbackIdx = 0;
          final Set<int> usedIndices = {};

          final subChapters = chapterData['subChapters'] as List? ?? [];
          for (var sub in subChapters) {
            if (sub is! Map) continue;
            final pages = sub['pages'] as List? ?? [];
            for (var page in pages) {
        ...
        lastError = e;
        debugPrint('Failed chapter generation with $currentModel: $e');
        final errStr = e.toString().toLowerCase();
        if (errStr.contains('api key not valid') ||
            errStr.contains('invalid api key') ||
            errStr.contains('permission_denied')) {
    ...
    throw lastError ??
        Exception(
          'Failed to generate chapter. Please ensure your Google Gemini API key is active.',
        );
  }

  static Future<Map<String, dynamic>> _executeChapterGeneration({
    required String modelName,
    required String apiKey,
    required String courseTitle,
    ...
    final existingChaptersNote = existingChapterTitles.isNotEmpty
        ? 'Existing Chapters already in this course: "${existingChapterTitles.join('", "')}". '
          'Do NOT repeat concepts from these previous chapters; smoothly progress to this new chapter topic.'
        : '';

    final systemPrompt = '''
You are an expert instructional designer and interactive curriculum architect for LearnByte.
Your task is to generate ONE comprehensive new Chapter to expand an existing course named "$courseTitle".
$existingChaptersNote

Curriculum Structure Rules:
1. Generate exactly 1 Chapter with title.
2. Scope & SubChapters:
   - Determine the optimal number of SubChapters (1 to 4 modules) needed to teach this specific chapter topic thoroughly.
   - For each SubChapter, generate 3 to 5 bite-sized mobile slide pages.
   - Assign xpReward: 10 for each SubChapter.
3. Slide Page Guidelines (Bite-sized mobile formatting):
   - Page 1: Concept Introduction & Key Definitions (text block with '# Heading', formatted bold keywords and bullets).
   $mediaGuideline
   - Final Page of each SubChapter MUST end with an interactive checkpoint:
     - 1 'text' summary block.
     - 1 'test' block with a targeted conceptual question, 3-4 options (1 or multiple is_correct), and a clear educational explanation.
4. Figure Matching Rule (when authentic figures are attached):
   - Inspect the numbered figures (Figure #1, Figure #2...) provided in this prompt.
   - For every 'media' block, set "figureIndex" to the exact 1-based number of the figure that matches the concept taught on that page.
   - Write a specific, descriptive "caption" directly explaining what is illustrated in that specific figure.
5. Target Audience Depth: $audience.
        ...
        },
        {
          ...
          "blocks": [
            ...
            },
            {
              ...
              "position": 2,
              "content": {
                "question": "Targeted conceptual question testing this module's core idea?",
                "options": [
                  {"text": "Correct Option A", "is_correct": true},
                  {"text": "Plausible Distractor B", "is_correct": false},
                  {"text": "Plausible Distractor C", "is_correct": false},
                  {"text": "Plausible Distractor D", "is_correct": false}
                ],
                "explanation": "Clear explanation of why option A is correct based on the lesson."
              }
            }
          ]
        }
...
''';
    ...
    if (attachment != null) {
      ...
      if (rawParts.isNotEmpty) {
        ...
        parts.add(
          TextPart(
            ...
            'Generate a new structured chapter for the course "$courseTitle" based on the attached file(s) (${fileName ?? "Uploaded study materials"}).\n'
            '${attachment.extractedImageUrls.isNotEmpty ? "Assign 'figureIndex' (1, 2, ...) in each 'media' block to match the relevant Figure with its slide.\n" : ""}'
            '${hasText ? "\nAdditional Extracted Study Content:\n${attachment.extractedText}\n" : ""}\n'
            'Chapter Focus / Instructions: ${promptOrInstructions.isEmpty ? "Cover next core concepts in depth" : promptOrInstructions}\n'
            'Audience Level: $audience',
          ),
        );
        contentParts.add(Content.multi(parts));
      ...
      contentParts.add(
        Content.text(
          ...
          'New Chapter Topic / Instructions: $promptOrInstructions\n'
          'Audience Level: $audience\n'
          'Please generate the complete chapter structure now in JSON format matching the schema.',
        ),
      );
    }
    ...
    final dynamic decoded = jsonDecode(cleanJson);
    if (decoded is! Map<String, dynamic>) {
      throw Exception('Invalid chapter structure received from AI.');
    }
